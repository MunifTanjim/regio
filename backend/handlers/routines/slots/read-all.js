...
const handler = asyncHandler(async (req, res, next) => {
  ...
  const queryOptions = {
    attributes: parseFieldsQuery(fields, ['RoutineSlotId']),
    order: parseSortQuery(sort, '+startTime'),
  ...
  const {
    count: totalItems,
    rows: routineSlots
  } = await RoutineSlot.findAndCountAll(queryOptions)

  const items = routineSlots.map(formatModelData)
