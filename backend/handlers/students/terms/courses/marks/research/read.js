...
const {
  models: { ResearchMark, Course, Student, Research }
...
const handler = asyncHandler(async (req, res, next) => {
  ...
  if (!req.grants.includes('sysadmin') && !req.grants.includes('head')) {
    ...
    if (req.grants.includes('teacher')) {
      const [adviserId, isSuperviser] = await Promise.all([
      ...
      if (!isSuperviser && adviserId !== getEntityIdFromUserId(UserId)) {
  ...
  const researchMark = await ResearchMark.findOne({
    where: {
      TermId,
      CourseId,
      StudentId
  ...
  const data = formatModelData(researchMark)
