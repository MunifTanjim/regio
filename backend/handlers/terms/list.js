...
const handler = asyncHandler(async (req, res, next) => {
  const { length = 8, page = 1, fields, filter, sort } = req.getData()

  const queryOptions = {
    attributes: parseFieldsQuery(fields),
    limit: length + 1,
    offset: (page - 1) * length,
    order: parseSortQuery(sort, '-SessionYearId,+BatchId,-level,-term'),
    where: parseFilterQuery(filter)
  }

  const { count: totalItems, rows: terms } = await Term.findAndCountAll(
    queryOptions
  )

  const items = terms.map(formatModelData)
