...
const {
  sequelize,
  models: { Address, ContactInfo, Country, Person, User }
...
const handler = asyncHandler(async (req, res, next) => {
  ...
  console.log(map(entity.User.Person.ContactInfos, 'ContactInfoId'))
  if (
    !entity ||
    !map(entity.User.Person.ContactInfos, 'ContactInfoId').includes(
      ContactInfoId
    )
  ) {
  ...
  await sequelize.transaction(async transaction => {
    await Promise.all([
      ContactInfo.update(
        { type, email: email ? normalizeEmail(email) : email, mobile, phone },
        { where: { ContactInfoId }, transaction }
      ),
      address
        ? Address.upsert({ ContactInfoId, ...address }, { transaction })
        : null
    ])
